...
test('handles zero cases', () => {
	const { getByText } = render(CountdownTimer, {
		endDate: mockNow.plus({ days: 1, hours: 0, minutes: 0, seconds: 0 }).toObject()
	});
...
test('handles empty cases', () => {
	const { getByText, queryByText } = render(CountdownTimer, {
		endDate: mockNow.plus({ days: 0, hours: 0, minutes: 0, seconds: 1 }).toObject()
	});
