...
export let shouldDownloadPhotos = (photos) =>  {
  let stalePercentage = 100 * (filterStalePhotos(photos).length / filterForDownloadedPhotos(photos).length);
...
export let filterPhotosForSearchTerms = (photos, searchTerms) => photos.filter(
